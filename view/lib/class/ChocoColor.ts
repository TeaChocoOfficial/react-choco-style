//-Path: "react-choco-style/lib/src/class/ChocoColor.ts"
import {
    ColorHex,
    ColorMain,
    ColorType,
...
import { ChocoStyleTypes } from '../types/choco';
import { UseChocoThemeType } from '../types/theme';

export class ChocoColor {
    private _theme: UseChocoThemeType;
    constructor(theme?: UseChocoThemeType) {
        this._theme = theme ?? useTheme();
    }
    get(color?: ColorsType): CColor {
        try {
    ...
    style({
        ...
        const { border } = this._theme.root.size;
        ...
        const disableds: ChocoStyleTypes = {
            clr: disabled,
        ...
        const focusStyles: ChocoStyleTypes = {
            css: {
                ':focus': {
