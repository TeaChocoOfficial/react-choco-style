...
import { atom, useRecoilState, useSetRecoilState, useRecoilValue, RecoilRoot } from 'recoil';
...
var themeModeAtom = atom({
    key: "theme mode",
    default: ChocoTheme.mode,
});

function useTheme() {
    var _a = useRecoilState(themeModeAtom), mode = _a[0], setMode = _a[1];
    var _b = useState(updateTheme()), theme = _b[0], setTheme = _b[1];
...
function ChocoStart(_a) {
    ...
    var setInner = useSetRecoilState(innerAtom);
    useEffect(function () {
...
function ChocoStyleToStyle(cs) {
    ...
    var inner = useRecoilValue(innerAtom);
    var _a = useState(getBreakpoint()), breakpoint = _a[0], setBreakpoint = _a[1];
    var _b = useState(getChocoStyle(cs)), css = _b[0], setCss = _b[1];
...
function Styled(tag) {
    return function (customStyles) {
        return function (props) {
            ...
            var Tag = tag;
            return jsx(Tag, __assign({}, newProps));
        };
    };
}
...
function ComponentSize(_a) {
    ...
    var _d = useState(size), value = _d[0], setValue = _d[1];
    useEffect(function () {
        if (typeof size === "number") {
...
function CSkeleton(prop) {
    ...
    props.children = (jsx(Skeleton, { posA: true, t: 0, l: 0, w: "20vh", h: "20vh", bgColor: "#ffffff22", style: {
            boxShadow: "0 0 10vh 10vh #ffffff22",
            animation: "CSkeleton 2s linear infinite",
        } }));
    return jsx(Skeleton, __assign({}, props));
}
...
function CText(prop) {
    ...
    if (skeleton) {
        ...
        return jsx(CSkeleton, __assign({}, props));
    }
    props.style = __assign(__assign({}, props.style), style);
    return jsx(Text, __assign({}, props));
}
...
function CPaper(prop) {
    ...
    return jsx(Paper, __assign({}, props));
}
...
function CButton(prop) {
    ...
    var _g = useState(false), isHover = _g[0], setIsHover = _g[1];
    var color = props.color, lowcase = props.lowcase, onClick = props.onClick, children = props.children;
    var _h = useState(false), isAnimating = _h[0], setIsAnimating = _h[1];
    ...
    return (jsxs(Button, __assign({}, props, { onMouseEnter: function () { return setIsHover(true); }, onMouseLeave: function () { return setIsHover(false); }, onClick: function (event) {
            if (onClick) {
                onClick(event);
            }
        ...
        }, children: [children, jsx(Effect, { bgColor: buttonColor === null || buttonColor === void 0 ? void 0 : buttonColor.action, h: formatSize(((_e = props.size) !== null && _e !== void 0 ? _e : 16) * 8), w: formatSize(((_f = props.size) !== null && _f !== void 0 ? _f : 16) * 8), style: {
...
function InitChoco(_a) {
    var children = _a.children;
    return (jsx(RecoilRoot, { children: jsx(ChocoStart, { children: children }) }));
}
...
export { CBox, CButton, CPaper, CSkeleton, CText, ChocoStart, ChocoStyleToStyle, ChocoTheme, ColorCommon, ColorDefault, ColorText, ComponentSize, GetColor, GetSetColor, InitChoco, KeywordsChocoStyle, KeywordsChocoStyleDef, KeywordsChocoStyleProps, Styled, callbackSize, chocoPropsToChocoStyle, formatSize, getFont, innerAtom, keysChocoStyle, keysChocoStyleProps, removeReservedProps, themeModeAtom, useTheme };
