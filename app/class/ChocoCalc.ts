...
import { GridType, StyleValue } from '../types/chocoValue';
...
type CssProperties =
    | [keyof SxType, SizesValue<string | number | undefined>]
    | undefined;
    ...
    _toSize<Value, NewValue extends string = 'none'>(
        ...
        if (Size.in(sizes)) {
            ...
            for (const [key, value] of breakpoints)
                if (getWindowInnerWidth() > value)
                    return sizes[key] as NewValue;
