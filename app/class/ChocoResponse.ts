...
import { CsType, CssType } from '../types/choco';
...
export class ChocoResponse {
    ...
    private _reduceCssProperties(css: CssType): SxType {
        return Obj.reduce<CssType, SxType>(
            ...
            (acc, key, value) => ({
                ...acc,
                [`&${key}`]: this._getCssProperties(this.chocoStyle(value).css),
            }),
            {},
        ) as SxType;
    }

    private _getCssProperties(styles: SxType): CssStyle {
        type CssProp = Record<keyof ChocoStyleDefType, SizesValue>;
        const css = new CssStyle();
        const prop = Obj.reduce<CssProp, CssProp>(
            { ...styles } as CssProp, 
            (acc, key, value) => {
                if (key === 'css')
                    return {
    ...
    chocoStyle(cs: CsType = {}): CssStyle {
        ...
        const CssProperties = this._getCssProperties(styles as SxType);
